...
object ClientConfigGenerator {
    ...
    /**
     * Shell dialect used for install commands: POSIX sh on macOS/Linux, cmd.exe on Windows.
     */
    ...
    fun generateConfig(
        ...
        return when (clientType) {
            ClientType.CLAUDE_CODE -> buildTerminalCommand(buildClaudeCodeCommand(serverUrl, serverName, platform), platform)
            ClientType.CODEX_CLI -> buildTerminalCommand(buildCodexCommand(serverUrl, serverName, platform), platform)
     ...
     * @param serverName Optional custom name for the server (defaults to IDE-specific name)
     * @param platform The command platform to use for the install command
    ...
    /**
     * Builds the process arguments used to execute an install command on the current platform.
     * POSIX runs through `sh -c`; Windows through `cmd.exe /d /c` (/d skips AutoRun hooks).
     */
     ...
     * On Windows the command is wrapped in an explicit `cmd.exe /d /c "..."` call so it works
     * when pasted into either cmd.exe or PowerShell (the redirects and `&` are cmd syntax).
     */
     ...
     */
    private const val LEGACY_SERVER_NAME = "jetbrains-debugger"
     ...
     * Also removes legacy server names from v1.x (jetbrains-debugger) to clean up after upgrade.
     ...
     * @param platform The command platform dialect to generate for
