...
object McpConstants {
    ...
    const val SERVER_DESCRIPTION = "JetBrains IDE debugger MCP server. Provides full debugger control including session management (start/stop debug sessions, run configurations), breakpoint management (set/remove/list breakpoints with conditions), execution control (step over/into/out, resume, pause, run to line), variable inspection (view/expand/modify variables, watches), expression evaluation, and stack/thread navigation. Use when you need to debug code programmatically, set breakpoints, inspect variables, or step through code execution. Requires an open JetBrains IDE project with debugger support."
